...
// FC (Finance Coordinator) signs in through this separate screen, not the
// shared role-tab login — there is no role selector here, only FC.
export default function LoginFC({ onLogin, sessionMsg }) {
  ...
  const submit = async () => {
    ...
    setBusy(true);
    try {
      const { token, user } = await api.login(u.trim(), p, "FC");
        ...
        <div className="slide-up" style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div className="row" style={{ gap: 12 }}>
            ...
            <div>
              ...
              <div className="dim" style={{ fontSize: 12 }}>Finance Coordinator</div>
            </div>
          </div>
          <ThemeToggle />
        </div>
